...
app.layout = html.Div([html.Div([dcc.Tabs(id='tabs',value='tab-1',children=[
                            ...
                            dcc.Tab(label='Kanały sprzedaży (dni)',value='tab-3'),
                            dcc.Tab(label='Kanały sprzedaży (płeć)',value='tab-4'),
                            ]),
...
def tab3_bar_store_type(chosen_cat):
    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['Store_type']==chosen_cat)].groupby(df.merged.tran_date.dt.day_name()).total_amt.sum().sort_values(ascending=False).to_frame()
    return go.Figure(data=go.Bar(x=grouped.index, y=grouped.total_amt))
